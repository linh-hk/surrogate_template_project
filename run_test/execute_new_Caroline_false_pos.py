...
Note: The imported data is changed to test for false positive rate in this script
...
def load_data(data_name):
    ...
    data = np.load(fi)
    return [[data[0,:,_],data[1,:,0 if _+1 == data.shape[2] else _+1]] for _ in np.arange(data.shape[2])]
...
if __name__=="__main__":
    ...
    with open(f'run_this_for_Caroline/{sys.argv[1]}/{tests}_falsepos_Linh.pkl', 'wb') as fi:
