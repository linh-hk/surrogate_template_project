...
sys.path.append('/home/k/khanhlinh-hoang/ItsASecret/Simulation_code/surrogate_dependence_test')
...
##########################################################################################
##########################################################################################
# Continue from failed runs
def check_config(saved, saveP):
    """True if the existing file's config matches the run we're about to do.
    Guards against a filename collision between two different configs."""
    s_cfg, n_cfg = saved.get("test_config", {}), saveP["test_config"]
    s_dg, n_dg   = saved.get("datagen_params", {}), saveP["datagen_params"]

    # plain-value checks (filename-determining settings)
    if s_cfg.get("pair_selection") != n_cfg.get("pair_selection"):
        return False
    if s_cfg.get("batch", {}).get("N0") != n_cfg.get("batch", {}).get("N0"):
        return False
    if s_dg.get("mode") != n_dg.get("mode"):
        return False

    # numpy-array checks (must use array_equal, not ==)
    for key in ("M", "s0", "mu"):
        a, b = s_dg.get(key), n_dg.get(key)
...
def check_streamed(path, N0, batch_N, saveP):
    frames = []
    truncated = False
    with open(path, "rb") as f:
        while True:
            try:
                ...
                break
    if not frames:
        log.warning(f"{path} is empty — starting fresh.")
        return False, N0
    if check_config(frames[0], saveP):
        if truncated:                        # rewrite clean frames, drop garbage tail
            with open(path, "wb") as f:
                for fr in frames:
                    pickle.dump(fr, f)
    
        results = frames[1:]                  # drop saveP config frame
        done_ids = set()
        for fr in results:
            sid = fr["FAILED"] if "FAILED" in fr else next(iter(fr.keys()))
            done_ids.add(int(sid))
    
        resume_at = N0
        while resume_at in done_ids and resume_at < N0 + batch_N:
            resume_at += 1
        CONTINUE = resume_at < N0 + batch_N
        return CONTINUE, resume_at

    else:
        raise SystemExit(
            f"{path} exists but its config does NOT match this run "
            f"(naming collision?). Refusing to resume. Check the filename / delete it."
        )
...
if __name__=="__main__":
    ...
    # data = data[N0 : N0 + batch_N] # goes to below after check_streamed
    ...
    if os.path.isfile(fiS):
        CONTINUE, resume_at = check_streamed(fiS, N0=N0, batch_N=batch_N, saveP=saveP)
    if CONTINUE:
        data = data[resume_at : N0 + batch_N]
        data = pair_selection(data=data, pair=pair, N0=resume_at)
        log.info(f'Continue at {fiS} from {resume_at}')
    else:
