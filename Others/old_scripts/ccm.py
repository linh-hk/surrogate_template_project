...
def ccm_loocv_old(data, embed_dim, tau=1, lib_sizes=[None], replace=False,
...
def choose_embed_params_old(ts, ed_grid=np.arange(1,9), tau_grid=np.arange(1,9), weights='exp', score=pcorr):
    ...
    for embed_dim in ed_grid:
        for tau in tau_grid:
            rho = ccm_loocv_old(data, embed_dim=embed_dim, tau=tau, pred_lag=1,
